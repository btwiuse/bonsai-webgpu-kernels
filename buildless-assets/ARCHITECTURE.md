...
`chat-header.css`, `chat-thread.css`, `chat-message.css`,
`chat-markdown.css`, `chat-composer.css`, `kernel-inspector.css`,
`app.css`) load in parallel with the scripts; order between CSS files
does not matter, but they must precede any module that touches
relevant DOM nodes.
...
`scene-background.js` is self-contained — the always-on glow + word
...
The `scene-` prefix is uniform across `background`, `prism`, and
`garden` so a single grep (`scene-*`) lists every Three.js concern.
...
Only `loader.js` writes to `window`. Scene modules are readers. The
full surface that ES modules may read off `window`:
